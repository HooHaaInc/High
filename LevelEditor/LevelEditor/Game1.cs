       ...
       protected override void Update(GameTime gameTime)
        {
            ...
            if ((ms.X > 0) && (ms.Y > 0) &&(ms.X < Camera.ViewPortWidth) &&(ms.Y < Camera.ViewPortHeight)){
                ...
                if (Camera.WorldRectangle.Contains((int)mouseLoc.X, (int)mouseLoc.Y)){
                    if (ms.LeftButton == ButtonState.Pressed && TileMap.EditorMode){
                    ...
                    (lastMouseState.RightButton == ButtonState.Released) && TileMap.EditorMode){
                        if (EditingCode){
                    ...
                    HoverCodeValue = TileMap.GetMapSquareAtCell(TileMap.GetCellByPixelX(
                    (int)mouseLoc.X),TileMap.GetCellByPixelY((int)mouseLoc.Y)).CodeValue;
                }
            }
            lastMouseState = ms;
